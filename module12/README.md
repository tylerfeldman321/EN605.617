...
## Goals
My goal with this assignment was to leverage OpenCL to create a useful application. Much of the code we look at as examples (like hello world style programs) performs trivial tasks. I wanted this assignment to provide some value to me. Specifically, I wanted to do audio processing, since that is a good use case for parallelization. I chose to write an OpenCL program to perform audio file panning, which involves adjusting the gain levels of a left vs. right channel.
...
## Triumphs
A big triumph was getting to hear the output of the program. It was very satisfying to play with the panning values and observe the program working as expecting and varying what I heard in the output. In writing the panning kernel, I learned a bit about different methods for panning (equal power vs. linear), and chose to do equal power panning.
